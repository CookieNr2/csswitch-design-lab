...
The layout follows the "split project files by feature or route" strategy from the [Next.js project structure guide](https://nextjs.org/docs/app/getting-started/project-structure#split-project-files-by-feature-or-route): `app/` holds the routes and the files only one route uses, and everything shared lives next to it.

Application code sits in the optional [`src` folder](https://nextjs.org/docs/app/api-reference/file-conventions/src-folder), which keeps it apart from the configuration files in the project root. As that page requires, `public/`, `.env*` files and config files such as `package.json`, `next.config.ts` and `tsconfig.json` stay in the root, and the `@/*` import path points at `src/`.
...
│   └── styles/               Global CSS: Tailwind and the shadcn/ui theme
...
- Cached queries are the `"use cache"` functions in `src/lib/server/catalog.ts` and `src/lib/server/configs.ts`.
